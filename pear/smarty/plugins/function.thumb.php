...
* Modyfikacje pluginu:
* Dodatkowe parametry:
* img="bool" - jeśli true, zwraca sam relatywny adres pliku z obrazkiem. Polecany do używania z link="false". Domyślnie wynosi false.
* square="bool" - jeśli true, obrazk przed przeskalowaniem jest nakładany na kwadrat o wybranym kolorze. Domyślnie wynosi false. Bok kwadratu jest równy długości najdłuższej krawędzi obrazka.
* square_color="html_color" - kolor kwadratu, na który nakładany jest obrazek przed skalowaniem. Format koloru -sześcioznakowy kod html(bez #). Domyślnie ffffff.
* Możliwe jest uzyskanie przeźroczystosci, wartość square_color musi wynosić wtedy "transparent".
* square_margin="int" - margines, jaki będzie dodany do krawędzi kwadratu. Domyślnie 0.
*/
...
function smarty_function_thumb($params, &$smarty)
{
	...
	{
		// Unsharp mask algorithm by Torstein Hřnsi 2003 (thoensi_at_netcom_dot_no)
		...
		function UnsharpMask($img, $amount, $radius, $threshold)
		{
			...
			$transparent = imagecolorallocatealpha($imgBlur, 255, 255, 255, 127);
			imagefilledrectangle($imgBlur, 0, 0, $w, $h, $transparent);
	...
	if (empty($params['cache']))
		$_CONFIG['cache'] = '/cache/images/';
	else
		$_CONFIG['cache'] = $params['cache'];

	$params['origfile'] = !empty($params['file']) ? $params['file'] : '';
	...
	{
		//$smarty->_trigger_fatal_error("thumb: parameter 'file' must exist");
		return '';
	}
	if (empty($params['greyscale'])) $params['greyscale']=false;
if (empty($params['square'])) $params['square']=false;
if (empty($params['square_margin'])) $params['square_margin']=0;
if (empty($params['square_color'])) $params['square_color']='ffffff';
	...
	if (empty($params['img'])) $params['img'] = false;
	if (empty($params['link'])) $params['link'] = true;
	if (empty($params['rel'])) $params['rel'] = '';
	if (empty($params['title'])) $params['title'] = '';
	if (empty($params['window'])) $params['window'] = false;
	...
	$_SRC['origfile']	= $params['origfile'];
	$_SRC['width']		= $temp[0];
	$_SRC['height']		= $temp[1];
	$_SRC['width_oryg']		= $temp[0];
	$_SRC['height_oryg']	= $temp[1];
	$_SRC['type']		= $temp[2]; // 1=GIF, 2=JPG, 3=PNG, SWF=4
	$_SRC['string']		= $temp[3];
	$_SRC['filename'] 	= basename($params['file']);
	$_SRC['modified'] 	= filemtime($params['file']);
$_SRC['longest_edge']=($temp[0]>$temp[1]?$temp[0]:$temp[1]);
$_SRC['shorter_edge']=($temp[0]<$temp[1]?$temp[0]:$temp[1]);
$_SRC['longest_edge']=  $_SRC['longest_edge']+$params['square_margin']*2; //bo margines to margines - z dwoch stron ma być
if ($params['square']==true){
	$_SRC['width']		= $_SRC['longest_edge'];
	$_SRC['height']		= $_SRC['longest_edge'];
}
if ($params['square_color']=='transparent') $params['type']=3;
	// Hash erstellen
	...
	{
		if (!@file_exists($params['frame']) && $params['frame'][0] == '/')
		{
			$params['frame'] = $_SERVER['DOCUMENT_ROOT'] . $params['frame'];
		}
	...
	$_RETURN['img'] = '<img src="'.$_DST['file'].'" '.$params['html'].' alt="'.$params['alt'].'" '.$_DST['string'].' />';

	if ($params['link'] == "true")
	{
		if (empty($params['linkurl']))
			$params['linkurl'] = $_SRC['origfile'];

		if ($params['window'] == "true")
			$returner = '<a href="' . $params['linkurl'] . '" rel="' . $params['rel'] . '" title="' . $params['title'] . '" target="_blank">' . $_RETURN['img'].'</a>';
		else
			$returner = '<a href="' . $params['linkurl'] . '" rel="' . $params['rel'] . '" title="' . $params['title'] . '">' . $_RETURN['img'] . '</a>';
	}
	else
	{
	...
	if ($params['square']==true)
	{
		$_TMP['square']=imagecreatetruecolor($_SRC['longest_edge'], $_SRC['longest_edge']);
		if ($params['square_color']!=='transparent')
		{
			$color=imagecolorallocatealpha($_TMP['square'],hexdec('0x' . $params['square_color']{0} . $params['square_color']{1}), hexdec('0x' . $params['square_color']{2} . $params['square_color']{3}), hexdec('0x' . $params['square_color']{4} . $params['square_color']{5}), 0);
		}
		else
		{
			$color=imagecolorallocatealpha($_TMP['square'], 0, 0, 0, 127);
		}
		imagefill($_TMP['square'], 0, 0, $color);

		$shorter_edge=(($_SRC['shorter_edge']==($_SRC['width_oryg']))?'width_oryg':'height_oryg');
		$coordinate=($_SRC['longest_edge']-$_SRC['shorter_edge'])/2;
		if ($shorter_edge=='width_oryg')
		{
		//  echo"szer";
			imagecopy($_TMP['square'], $_SRC['image'],  $coordinate, $params['square_margin'],0, 0, $_SRC['width_oryg'],$_SRC['height_oryg']);
			}
		elseif ($shorter_edge=='height_oryg')
			{
		//    echo"wys";
			imagecopy($_TMP['square'], $_SRC['image'],  $params['square_margin'], $coordinate, 0, 0, $_SRC['width_oryg'],$_SRC['height_oryg']);
			}
		else
		{
			imagecopy($_TMP['square'], $_SRC['image'], $params['square_margin'], $params['square_margin'], 0, 0, $_SRC['width_oryg'],$_SRC['height_oryg']);
		}
		$_SRC['image']=$_TMP['square'];
		unset($_TMP['square']);
	...
	$transparent = imagecolorallocatealpha($_DST['image'], 255, 255, 255, 127);
	imagefilledrectangle($_DST['image'], 0, 0, $_DST['width'], $_DST['height'], $transparent);
	...
	{
		$bwimage= imagecreate($_DST['width'], $_DST['height']); //Creates the 256 color palette
		$palette = array();
		for ($c=0;$c<256;$c++)
		{
		...
		for ($y=0;$y<$_DST['height'];$y++)
		{
			...
			{
				$rgb = imagecolorat($_DST['image'],$x,$y);
				$r = ($rgb >> 16) & 0xFF;
				$g = ($rgb >> 8) & 0xFF;
				$b = $rgb & 0xFF;
				$gs = thumb_yiq($r,$g,$b);
	...
	{
		if (!@file_exists($params['overlay']) && $params['overlay'][0] == '/')
		{
			$params['overlay'] = $_SERVER['DOCUMENT_ROOT'] . $params['overlay'];
		}
	...
	if ($_DST['type'] == 1)
	{
		...
		imagegif($_DST['image'], $_SERVER['DOCUMENT_ROOT'] . $_DST['file']);
	}
	if ($_DST['type'] == 2)
	{
		Imageinterlace($_DST['image'], 1);
		if (empty($params['quality'])) $params['quality'] = 80;
		imagejpeg($_DST['image'], $_SERVER['DOCUMENT_ROOT'] . $_DST['file'], $params['quality']);
	}
	if ($_DST['type'] == 3)
	{
		imagepng($_DST['image'], $_SERVER['DOCUMENT_ROOT'] . $_DST['file']);
	}
...
function thumb_yiq($r,$g,$b) { return (($r*0.299)+($g*0.587)+($b*0.114)); }
